defmodule Ineedthis.Users.UserToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  def build_session_token(user) do
    ...
    {token, %Ineedthis.Users.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  def build_totp_token(user) do
    token = :crypto.strong_rand_bytes(@rand_size)
    {token, %Ineedthis.Users.UserToken{token: token, context: "totp", user_id: user.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Ineedthis.Users.UserToken, where: [token: ^token, context: ^context]
  end
  ...
  def user_and_contexts_query(user, :all) do
    from t in Ineedthis.Users.UserToken, where: t.user_id == ^user.id
  end

  def user_and_contexts_query(user, [_ | _] = contexts) do
    from t in Ineedthis.Users.UserToken, where: t.user_id == ^user.id and t.context in ^contexts
  end
end
