defmodule IneedthisWeb.Api.Json.FilterView do
  use IneedthisWeb, :view

  def render("index.json", %{filters: filters, total: total} = assigns) do
    %{
      filters: render_many(filters, IneedthisWeb.Api.Json.FilterView, "filter.json", assigns),
      total: total
    }
  end

  def render("show.json", %{filter: filter} = assigns) do
    %{filter: render_one(filter, IneedthisWeb.Api.Json.FilterView, "filter.json", assigns)}
  end
